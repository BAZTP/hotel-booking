...
$stmt = $mysqli->prepare("UPDATE bookings SET status='paid', paid_at=NOW() WHERE booking_code=? AND status='pending'");
...
if($stmt->affected_rows<=0){
  ...
  if($b['status']==='paid') json_out(["ok"=>true,"status"=>"paid"]);
  json_out(["ok"=>false,"error"=>"No se puede pagar (estado: ".$b['status'].")"],400);
}
